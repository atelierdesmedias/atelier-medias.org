...
export class HomePage extends DOMView {
  ...
  protected prepareDependencies() {
    ...
    this._coworkersPresentation = new CoworkersPresentation(
      this.$root.find('.CoworkersPresentation')
    );

    // inclure coworkersPatchwork
    this._coworkersPatchworkSection = new CoworkersPatchwork(
      this.$root.find('.CoworkersPatchwork')
    );

    // inclure le texte de présentation
    this._admPresentation = new AdmPresentation(
      this.$root.find('.AdmPresentation')
    );

    // inclure la section "adherent"
    this._adherentSection = new AdherentSection(
      this.$root.find('.AdherentSection')
    );

    // inclure features section
    this._featuresSection = new FeaturesSection(
      this.$root.find('.FeaturesSection')
    );
