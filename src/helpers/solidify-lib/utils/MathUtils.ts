export class MathUtils {
  ...
  static normalizeAngle(pAngle: number): number {
    return MathUtils.positiveModulo(pAngle + Math.PI, Math.PI * 2) - Math.PI;
  }
  ...
  static randomPickFromArray<P>(pArray: P[]): P {
    ...
    return pArray[MathUtils.randomRange(0, pArray.length, true)];
  }
   ...
   *
   */
  static functionalValueMap<GType>(
    valuesMap: ((r: GType) => GType | boolean)[][]
  ) {
